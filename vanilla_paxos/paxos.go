...
func (p *Paxos) P1a(r paxi.Request) {
	...
	p.Broadcast(P1a{Ballot: p.log[p.slot].ballot, Slot: slotTmp})
...
func (p *Paxos) HandleP1a(m P1a) {
	...
	p.Send(m.Ballot.ID(), P1b{
		Ballot:  e.ballot,
		ID:      p.ID(),
		Slot:    m.Slot, // line added by me to fix the slot not found problem
		Command: e.command,
	})
}
...
func (p *Paxos) exec() {
	for {
		...
		// TODO clean up the log periodically. Removed the below line for now. Figure out its consequences
